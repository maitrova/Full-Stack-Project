...
const serializeCombo = (comboDoc) => {
  ...
  return {
    ...combo,
    includedProductsCount: combo.items?.length || 0,
    productImages: selectedProductImages,
    displayImage:
      combo.imageMode === "CUSTOM_IMAGES"
        ? combo.featuredImage || combo.galleryImages?.[0]?.url || combo.bannerImage || selectedProductImages[0] || null
        : selectedProductImages[0] || combo.featuredImage || null,
    pricing,
...
const validateComboPayload = async ({ items, comboPrice, originalPriceOverride }) => {
...
export const createComboPack = async (req, res) => {
  try {
    ...
    const items = normalizeProductItems(req.body.items || req.body.productIds, allowDuplicateProducts);
    const comboPrice = Number(req.body.comboPrice);
    const originalPriceOverride = parseOptionalNumber(req.body.originalPrice);

    await validateComboPayload({ items, comboPrice, originalPriceOverride });
